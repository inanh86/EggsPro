...
class Customize {
    ...
    public function init() {
        $this->include();
        add_action('customize_register', [$this, 'dang_ky_customize'] );
        add_action('customize_preview_init', [$this, 'customize_scripts'] );
    }
    public function include() {
        // Resouce API
        include_once dirname( __FILE__ )  . '/ux/class-slide.php';
    }
    /**
     * đăng ký customize
     */
    public function dang_ky_customize($custom) {
        $custom->remove_panel( 'woocommerce' );
        $custom->remove_panel( 'themes' );
        ////
        $this->main_customize($custom); // Edit main logo
        \EggsPro\src\UX\SlideBanner::init($custom); // slidebar
    }
    public function customize_scripts() {
        
    }
    public function main_customize($customize) {
        // Thay đổi section logo mặc định
        $customize->add_section( 'title_tagline', 
            array(
                'title'       => __lang('Chỉnh sửa Logo'), //Visible title of section
                'priority'    => 35, //Determines what order this appears in
                'capability'  => 'edit_theme_options', //Capability needed to tweak
                'description' => __lang('Chỉnh sửa logo, tiêu đề trang chủ, logo website, biểu tượng của trang'), //Descriptive tooltip
            ) 
        );
